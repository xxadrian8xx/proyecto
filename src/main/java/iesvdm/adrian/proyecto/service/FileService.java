...
public class FileService implements StorageService{
    ...
    public void init() throws IOException {
        rootLocation = Paths.get(imagenLocation);
    ...
    public String store(MultipartFile file) {
        try {
            ...
            Path destinatonFile = rootLocation.resolve(Paths.get(filename))
                    .normalize().toAbsolutePath();
            try(InputStream inputStream = file.getInputStream()) {
                Files.copy(inputStream, destinatonFile, StandardCopyOption.REPLACE_EXISTING);
            }
            return filename;
        } catch (IOException e){
